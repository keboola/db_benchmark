...
foreach ($DWU in $DWUs)
{
    ...
    $resultfile = "$PSScriptRoot\results\AzureResults_CTAS_$($DWU)_$timestamp.csv"
    ...
    for ($i = 0; $i -lt $iterations; $i++)
    {
        if ($ClearCache) {
            ...
            Invoke-Sqlcmd -ServerInstance "$ServerName.database.windows.net" -Database $DatabaseName -UserName $UserName -Password $Password -InputFile $clearCacheFile 
        }

        $warmupfiles = Get-ChildItem $PSScriptRoot\Microsoft_sql -Filter warmup_*.sql | Sort-Object -Property Name
        $queryfiles = Get-ChildItem $PSScriptRoot\Microsoft_sql_CTAS -Filter query*.sql | Sort-Object -Property Name
        ...
        foreach ($queryfile in $queryfiles)
        {
            ...
            $tableName = "[tst].[" + $queryfile.BaseName + "]"
            Invoke-Sqlcmd -ServerInstance "$ServerName.database.windows.net" -Database $DatabaseName -UserName $UserName -Password $Password -Query "IF OBJECT_ID('$tableName') IS NOT NULL DROP TABLE $tableName"  
