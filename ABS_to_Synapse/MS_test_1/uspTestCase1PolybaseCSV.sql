alter proc TestCase1PolybaseCSV
	@DWU int,
...
IF OBJECT_ID(''[ext].[lineitems_tmp]'') IS NOT NULL
	DROP EXTERNAL TABLE [ext].[lineitems_tmp];
CREATE EXTERNAL TABLE [ext].[lineitems_tmp] (
	[L_ORDERKEY] nvarchar(4000) ,
	[L_PARTKEY] nvarchar(4000) ,
	[L_SUPPKEY] nvarchar(4000) ,
	[L_LINENUMBER] nvarchar(4000) ,
...
WITH
(
	LOCATION=''' + @folder + '/' + @compression + '/' + @scalefactorfolder + '/' + @filesfolder + '/'',
    DATA_SOURCE = DbxshareDataSource ,
    --DATA_SOURCE = KeboolaBlobDataSource ,
    FILE_FORMAT = KeboolaTextFileFormat
) ;

--IF OBJECT_ID(''[dbo].[lineitems_tmp]'') IS NOT NULL
--	DROP TABLE [dbo].[lineitems_tmp]
IF OBJECT_ID(''tempdb..#lineitems_tmp'') IS NOT NULL
	DROP TABLE #lineitems_tmp

--CREATE TABLE [dbo].[lineitems_tmp]
CREATE TABLE #lineitems_tmp
WITH (HEAP, DISTRIBUTION = HASH(L_ORDERKEY) )
--WITH (CLUSTERED COLUMNSTORE INDEX, DISTRIBUTION = HASH(L_ORDERKEY) )
--WITH (DISTRIBUTION = HASH(L_ORDERKEY), CLUSTERED INDEX(L_ORDERKEY))
AS SELECT * FROM [ext].[lineitems_tmp]
...
IF OBJECT_ID(''tempdb..#lineitems_dedupe'') IS NOT NULL
	DROP TABLE #lineitems_dedupe
...
select getdate(), ' + convert(varchar,@DWU) + ', ''' + @resourceclass + ''' as resourceclass, ''Polybase'' as Method, 
...
/*
exec TestCase1PolybaseCSV @DWU=0, @ResourceClass='xlargerc', @scalefactor=10, @files=200, @folder='CSV', @compression='none'
exec TestCase1PolybaseCSV @DWU=0, @ResourceClass='xlargerc', @scalefactor=10, @files=200, @folder='CSV', @compression='none'
exec TestCase1PolybaseCSV @DWU=0, @ResourceClass='xlargerc', @scalefactor=100, @files=200, @folder='CSV', @compression='none'
exec TestCase1PolybaseCSV @DWU=0, @ResourceClass='xlargerc', @scalefactor=100, @files=200, @folder='CSV', @compression='none'

exec TestCase1PolybaseCSV @DWU=500, @ResourceClass='largerc', @scalefactor=1, @files=60, @folder='CSV', @compression='gzip'
exec TestCase1PolybaseCSV @DWU=500, @ResourceClass='largerc', @scalefactor=1, @files=60, @folder='CSV', @compression='none'
*/
